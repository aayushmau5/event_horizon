...
(function() {
  ...
  class Neko {
    ...
    // Recolor sprites using Canvas pixel manipulation + flood fill.
    // The original sprites are pure binary: black outlines on transparent backgrounds.
    // We flood-fill from all edges to find "outside" pixels, then everything enclosed
    // by the outline is "inside" (fur). Black pixels become the outline color, inside
    // pixels become fur color, and white highlight dots stay white.
    function recolorSprites(sprites, furColor, outlineColor) {
        const S = SPRITE_SIZE;

        function floodFillOutside(data) {
            // BFS flood fill from all edge pixels that are transparent (alpha <= 1)
            // Marks all reachable transparent pixels as "outside"
            ...
            const enqueue = (x, y) => {
                ...
                const p = idx * 4;
                // Can walk through transparent or ghost pixels (alpha <= 1)
                if (data[p + 3] <= 1) {
                    outside[idx] = 1;
                    queue.push(x, y);
                }
            ...
            for (let y = 0; y < S; y++) { enqueue(0, y); enqueue(S - 1, y); }
        ...
        return sprites.map(src => {
            return new Promise(resolve => {
                const img = new Image();
                img.onload = () => {
                    // Each sprite gets its own canvas to avoid race conditions
                    ...
                    const outside = floodFillOutside(data);
                    ...
                    for (let i = 0; i < S * S; i++) {
                        ...
                        if (a <= 1) {
                            // Transparent / ghost pixel
                            data[p + 3] = 0;
                        } else if (r < 128) {
                            // Black outline pixel → dark outline color
                            data[p]     = outlineColor[0];
                            data[p + 1] = outlineColor[1];
                            data[p + 2] = outlineColor[2];
                            data[p + 3] = 255;
                        } else if (!outside[i]) {
                            // White pixel enclosed inside the outline → fur color
                            data[p]     = furColor[0];
                            data[p + 1] = furColor[1];
                            data[p + 2] = furColor[2];
                            data[p + 3] = 255;
                        } else {
                            // White pixel outside the outline → keep white (eye shine etc.)
                            data[p]     = 255;
                            data[p + 1] = 255;
                            data[p + 2] = 255;
                            data[p + 3] = 255;
    ...
    // Parse a CSS hex/rgb color string into [r, g, b]
    function parseCssColor(str) {
        ...
        return [33, 218, 162]; // default theme-one fallback
    }
    ...
    // Get the position the cat should sleep at (bottom-left of viewport)
    function getNavbarSleepPosition() {
        return {
            x: 8,
            y: window.innerHeight - SPRITE_SIZE - 8,
        };
    }

    // Auto-initialize function
    window.createNeko = function(options) {
        // Start the cat at the navbar position, sleeping
        const sleepPos = getNavbarSleepPosition();
        const { startSleeping: _ignored, ...nekoOptions } = options || {};
        const neko = new Neko({
            ...nekoOptions,
            startX: sleepPos.x,
            startY: sleepPos.y,
        ...
        neko.element.style.pointerEvents = "auto";
        neko.element.style.cursor = "pointer";
        ...
        neko.logicX = sleepPos.x;
        neko.logicY = sleepPos.y;
        neko.prevLogicX = sleepPos.x;
        neko.prevLogicY = sleepPos.y;
        neko.x = sleepPos.x;
        neko.y = sleepPos.y;
        neko.targetX = sleepPos.x + SPRITE_SIZE / 2;
        neko.targetY = sleepPos.y + SPRITE_SIZE - 1;
        neko.oldTargetX = neko.targetX;
        neko.oldTargetY = neko.targetY;

        // When sleeping/returning: override chaseMouse to keep her pinned
        const sleepingChaseMouse = function() {
            const p = getNavbarSleepPosition();
            this.runTowards(p.x + SPRITE_SIZE / 2, p.y + SPRITE_SIZE - 1);
        };
        ...
        const goToSleep = () => {
            ...
            awake = false;
            returning = true;
            // Override chaseMouse to run back to sleep position, then pin her there
            neko.chaseMouse = function() {
                const p = getNavbarSleepPosition();
                this.runTowards(p.x + SPRITE_SIZE / 2, p.y + SPRITE_SIZE - 1);
                if (this.moveDX === 0 && this.moveDY === 0) {
                    returning = false;
                    this.setState(NekoState.SLEEP);
                    this.chaseMouse = sleepingChaseMouse;
                }
            };
        };

        // Click neko → toggle (if returning mid-journey, cancel and wake)
        ...
        // Expose for app.js to drive on navigation
        neko.wakeUp = wakeUp;
        neko.goToSleep = goToSleep;

        neko.applyThemeColor = function() {
            const style = getComputedStyle(document.documentElement);
            const themeColor = style.getPropertyValue("--theme-one");
            const furColor = parseCssColor(themeColor);
            const outlineColor = darken(furColor, 0.45);

            Promise.all(recolorSprites(NEKO_SPRITES, furColor, outlineColor)).then(recolored => {
                neko.setSprites(recolored);
            });
        };

        neko.applyThemeColor();
        neko.start();
